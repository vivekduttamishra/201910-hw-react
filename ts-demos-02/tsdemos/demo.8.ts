...
//there should be an object containing
//a function called search with given signature
// interface ICondition<T>{
//      search(value:T,param:any):boolean;
// }

//there should be function with given signature
//no class is required to implement it
interface ICondition<T>{
    (record:T,param:any):boolean;
}
...
class GenericSearchEngine<T>{
    ...
    search(condition:ICondition<T>,param:any):T[]{
        var result:T[]=[];
...
var se=new GenericSearchEngine<Book>(books);

var matchAuthor:ICondition<Book>=function(book:Book,authorName:string){
